...
function findLocalOnlyItems<T extends { id: string }>(
  localItems: T[],
  cloudItems: T[] | null
): T[] {
...
export function useLocalNotes(userId?: string | null) {
  ...
  useEffect(() => {
    ...
    const notesChannel = supabase
      ...
      .on(
        ...
        async payload => {
          await handleNoteRealtimeEvent(payload, setNotes);
        }
      )
      .subscribe();
    ...
    const foldersChannel = supabase
      ...
      .on(
        ...
        async payload => {
          await handleFolderRealtimeEvent(payload, setFolders);
        }
      )
      .subscribe();
  ...
  const deleteFolder = async (folderId: string) => {
    try {
      ...
      if (!folder) {
        return { error: new Error('Folder not found') };
      }
  ...
  const restoreFolder = async (folderId: string) => {
    try {
      ...
      const deletedNotesInFolder = notes.filter(n => n.folder_id === folderId && n.deleted_at);
