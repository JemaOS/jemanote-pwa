...
test.describe('Authentication', () => {
  ...
  test.describe('User Registration', () => {
    test('should register a new user successfully', async ({ page }) => {
      ...
      await expect(page.getByRole('button', { name: /déconnexion|logout/i })).toBeVisible({ timeout: 10000 })
    })
    ...
    test('should show error for duplicate email', async ({ page }) => {
      ...
      await expect(page.getByText(/existe déjà|already exists|déjà utilisé/i)).toBeVisible({ timeout: 5000 })
    })
  })
  ...
  test.describe('User Login', () => {
    test('should login with valid credentials', async ({ page }) => {
      ...
      await page.getByRole('button', { name: /connexion|login/i }).first().click()
    ...
    test('should show error for invalid credentials', async ({ page }) => {
      ...
      await expect(page.getByText(/identifiants invalides|invalid credentials|email ou mot de passe incorrect/i)).toBeVisible({ timeout: 5000 })
    })
  ...
  test.describe('User Logout', () => {
    test('should logout successfully', async ({ page }) => {
      ...
      await expect(page.getByRole('button', { name: /connexion|login/i })).toBeVisible({ timeout: 10000 })
    })
  })
  ...
  test.describe('Password Recovery', () => {
    test('should show password recovery form', async ({ page }) => {
      ...
      const forgotPasswordLink = page.getByRole('button', { name: /mot de passe oublié|forgot password/i })
      ...
      if (await forgotPasswordLink.isVisible().catch(() => false)) {
        ...
        await expect(page.getByPlaceholder(/email/i)).toBeVisible()
        await expect(page.getByRole('button', { name: /envoyer|send|réinitialiser/i })).toBeVisible()
      } else {
    ...
    test('should send password reset email', async ({ page }) => {
      ...
      if (await forgotPasswordLink.isVisible().catch(() => false)) {
        ...
        await expect(page.getByText(/email envoyé|email sent|vérifiez votre boîte/i)).toBeVisible({ timeout: 5000 })
      } else {
        test.skip()
      }
    })
  ...
  test.describe('Session Persistence', () => {
    ...
    test('should persist session in new tab', async ({ browser }) => {
      ...
      await expect(newPage.getByRole('button', { name: /déconnexion|logout/i })).toBeVisible({ timeout: 10000 })
  ...
  test.describe('Auth Modal UI', () => {
    ...
    test('should switch between login and register tabs', async ({ page }) => {
      ...
      await expect(page.getByRole('button', { name: /connexion|login/i }).first()).toHaveClass(/bg-primary/)

      // Switch to register
      await page.getByRole('button', { name: /inscription|sign up/i }).click()

      // Check register tab is active
      await expect(page.getByRole('button', { name: /inscription|sign up/i })).toHaveClass(/bg-primary/)
    })
  })
})
