...
export const mistralHandlers = [
  ...
  http.post(
    ...
    async ({ request }) => {
      ...
      if (userMessage.toLowerCase().includes('résume') || userMessage.toLowerCase().includes('summary')) {
        responseContent = 'Ceci est un résumé concis du texte fourni, mettant en avant les points clés.'
      } else if (userMessage.toLowerCase().includes('continue') || userMessage.toLowerCase().includes('texte à continuer')) {
        responseContent = 'Voici la continuation naturelle et cohérente du texte, développant les idées présentées.'
      } else if (userMessage.toLowerCase().includes('améliore') || userMessage.toLowerCase().includes('improve')) {
        responseContent = 'Voici une version améliorée, plus claire et mieux structurée du texte original.'
      } else if (userMessage.toLowerCase().includes('traduis') || userMessage.toLowerCase().includes('translate')) {
        responseContent = 'This is the translated version of the provided text.'
      } else if (userMessage.toLowerCase().includes('tags') || systemMessage.toLowerCase().includes('tags')) {
        responseContent = 'productivité, organisation, notes, idées, projet'
      } else if (userMessage.toLowerCase().includes('idées') || userMessage.toLowerCase().includes('brainstorming')) {
        responseContent = 'Idée 1: Explorer de nouvelles approches\nIdée 2: Collaborer avec l\'équipe\nIdée 3: Documenter les processus\nIdée 4: Automatiser les tâches répétitives\nIdée 5: Mesurer les résultats'
      } else if (userMessage.toLowerCase().includes('synthétise') || userMessage.toLowerCase().includes('synthesize')) {
...
export const supabaseAuthHandlers = [
  ...
  // Get user
  http.get(
    'https://yadtnmgyrmigqbndnmho.supabase.co/auth/v1/user',
    async ({ request }) => {
      await delay(50)
  ...
  // Sign out
  http.post(
    'https://yadtnmgyrmigqbndnmho.supabase.co/auth/v1/logout',
    async () => {
      await delay(50)
...
export const supabaseDatabaseHandlers = [
  // Notes CRUD
  http.get(
    'https://yadtnmgyrmigqbndnmho.supabase.co/rest/v1/notes',
    async ({ request }) => {
      await delay(100)
      const url = new URL(request.url)
  ...
  // Folders CRUD
  http.get(
    'https://yadtnmgyrmigqbndnmho.supabase.co/rest/v1/folders',
    async ({ request }) => {
      await delay(100)
      const url = new URL(request.url)
      const userId = url.searchParams.get('user_id')
  ...
  // Tags CRUD
  http.get(
    'https://yadtnmgyrmigqbndnmho.supabase.co/rest/v1/tags',
    async ({ request }) => {
      await delay(100)
      const url = new URL(request.url)
      const userId = url.searchParams.get('user_id')
  ...
  // Links CRUD
  http.get(
    'https://yadtnmgyrmigqbndnmho.supabase.co/rest/v1/links',
    async ({ request }) => {
      await delay(100)
      const url = new URL(request.url)
