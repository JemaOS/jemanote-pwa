...
describe('Supabase Realtime Integration', () => {
  ...
  describe('Connection Management', () => {
    it('should handle subscription status changes', async () => {
      ...
      channel = supabase.channel('status_test').subscribe(status => {
  ...
  describe('Error Handling', () => {
    it('should handle subscription errors', async () => {
      server.use(
        http.get('https://yadtnmgyrmigqbndnmho.supabase.co/realtime/v1/websocket', async () => {
          return HttpResponse.json({ error: 'Connection refused' }, { status: 403 });
        })
      );

      const errorCallback = vi.fn();

      channel = supabase.channel('error_test').subscribe((status, err) => {
        if (err) {
          errorCallback(err);
        }
      });
    ...
    it('should handle network disconnection', async () => {
      ...
      channel = supabase.channel('network_test').subscribe(status => {
  ...
  describe('Broadcast Events', () => {
    it('should send broadcast message', async () => {
      channel = supabase.channel('broadcast_test').subscribe();
  ...
  describe('Presence', () => {
    ...
    it('should untrack presence', async () => {
      channel = supabase.channel('presence_untrack').subscribe();
