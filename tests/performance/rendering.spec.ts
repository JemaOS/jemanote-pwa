...
async function getPaintMetrics(page: Page): Promise<PaintMetrics> {
  return page.evaluate(() => {
    ...
    const lcp = (lcpEntries[lcpEntries.length - 1] as PerformanceEntry & { startTime: number })?.startTime || 0;
  ...
  test.describe('Editor Rendering', () => {
    test('typing should not cause frame drops', async ({ page }) => {
      ...
      if (await editor.isVisible().catch(() => false)) {
        ...
        await page.keyboard.type('The quick brown fox jumps over the lazy dog. '.repeat(10), { delay: 10 });
