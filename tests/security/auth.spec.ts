...
test.describe('Authentication Security', () => {
  ...
  test('should not store passwords in localStorage', async ({ page }) => {
    ...
    const passwordKeys = localStorageKeys.filter(key => 
  ...
  test('should clear sensitive data on logout', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      localStorage.setItem('user-session', JSON.stringify({ id: '123', email: 'test@example.com' }))
  ...
  test('should have secure token storage', async ({ page }) => {
    ...
    const storageInfo = await page.evaluate(() => {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i)
        if (key) {info.localStorage.push(key)}
      }
      
      for (let i = 0; i < sessionStorage.length; i++) {
        const key = sessionStorage.key(i)
        if (key) {info.sessionStorage.push(key)}
      }
    ...
    const hasTokenInLocalStorage = storageInfo.localStorage.some(key =>
  ...
  test('should validate redirect URLs after login', async ({ page }) => {
    ...
    for (const redirect of maliciousRedirects) {
      const isAllowed = allowedDomains.some(domain => 
        redirect.includes(domain) && redirect.startsWith('https://')
      )
