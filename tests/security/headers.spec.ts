...
test.describe('HTTP Security Headers', () => {
  ...
  test('should have X-Content-Type-Options: nosniff', async ({ page }) => {
    const response = await page.waitForResponse(resp => 
      resp.url() === page.url() && resp.status() === 200
    )
  ...
  test('should prevent MIME type sniffing on all resources', async ({ page }) => {
    ...
    // Check main document
    const mainResponse = await page.waitForResponse(resp => 
      resp.url() === page.url() && resp.status() === 200
    )
  ...
  test('should have Content-Security-Policy or Content-Security-Policy-Report-Only', async ({ page }) => {
    const response = await page.waitForResponse(resp => 
      resp.url() === page.url() && resp.status() === 200
    )
