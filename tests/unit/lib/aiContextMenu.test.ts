...
describe('aiContextMenu', () => {
  let onShowMenuMock: ReturnType<typeof vi.fn> &
    ((position: { x: number; y: number; selectedText: string }) => void);

  beforeEach(() => {
    onShowMenuMock = vi.fn() as unknown as ReturnType<typeof vi.fn> &
      ((position: { x: number; y: number; selectedText: string }) => void);
    vi.clearAllMocks();
  });
  ...
  describe('aiContextMenuExtension', () => {
    ...
    it('should prevent default context menu behavior', () => {
      ...
      const { handlers } = extension as unknown as {
        handlers: { contextmenu: (event: any, view: any) => boolean };
      };
