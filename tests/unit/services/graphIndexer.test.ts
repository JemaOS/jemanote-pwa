...
describe('graphIndexer', () => {
  ...
  describe('indexGraph', () => {
    ...
    it('should create backlinks for bidirectional connections', () => {
      ...
      const anyEdge = result.edges.find((e: GraphEdge) => e.from === 'note-2' && e.to === 'note-1');
    ...
    it('should resolve links by title case-insensitively', () => {
      ...
      const edge = result.edges.find((e: GraphEdge) => e.from === 'note-1' && e.to === 'target-id');
      expect(edge).toBeDefined();
    });
  ...
  describe('getBacklinks', () => {
    it('should return empty array for isolated node', () => {
      const edges: GraphEdge[] = [{ from: 'note-1', to: 'note-2', type: 'wikilink' }];
